...
# 外部コーチ2名は部分参加のため、日程確定次第別途連絡する（本表には含めない）。
...
for i, day in enumerate(days):
    b = '－' if i == 0 else base_am_pm[i] + ADVISORS
    l = base_am_pm[i] + ADVISORS
    if base_dinner[i] is None:
        d = '－'
        s = '－'
    else:
        d = base_dinner[i] + ADVISORS
        s = base_dinner[i] + ADVISORS
...
set_cell(table1, 2, 1, '2名　（参加日程は確定次第、別途ご連絡いたします）', highlight=True)
set_cell(table1, 3, 0, '合計（本状での確定分）', bold=True)
set_cell(table1, 3, 1, '44名　（部員42名＋顧問2名。コーチ分は別途加算）')
...
# ── 日別人数表 ────────────────────────────────────────
para('４．日別人数表（部員・顧問分／外部コーチを除く）', bold=True, size=11, space_before=4, space_after=2)
...
run_n2 = note2.add_run(
    '※　上表は部員42名・引率顧問2名の確定分のみを反映しております。'
    '外部コーチ2名分の食事・宿泊数につきましては、参加日程確定後に別途ご連絡いたしますので、'
    '恐れ入りますが上表に加算のうえご準備いただけますと幸いです。'
)
run_n2.font.size = Pt(9.5)
run_n2.font.name = '游明朝'
run_n2._element.rPr.rFonts.set(qn('w:eastAsia'), '游明朝')
highlight_run(run_n2)

para('５．お願い', bold=True, size=11, space_before=4, space_after=2)
body3 = doc.add_paragraph()
body3.paragraph_format.space_after = Pt(8)
run3 = body3.add_run(
    '上記人数にもとづき、お食事の提供数調整をお願いできますと幸いです。'
    'なお、最終のご請求額につきましては、合宿終了後の実績人数（外部コーチ分を含む）にもとづき、'
